...
| 로그인 응답 속도 | 성공 로그인 약 5초 지연을 Supabase 인증, 사용자 조회, JDBC session 저장, quota 조회 구간으로 나누어 측정하고 중복 조회, CSRF 선요청과 DB 연결 재사용을 최적화한다. | 미해결, 운영 구간 측정 필요 |
| 다중 장르 분류 | iTunes의 단일 장르만 사용하는 현재 방식을 보완해 곡별 복수 장르, 운영자 `CURATED` 분류, 대표 장르와 장르별 차트 포함 기준을 설계한다. | 미해결, 제품 정책 결정 필요 |
