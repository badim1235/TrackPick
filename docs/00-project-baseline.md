...
| 가입 연령 | 생년월일을 수집하거나 별도 연령 제한을 두지 않는다. |
| 개인정보 보관 | 가입 제한용 IP hash는 24시간 이내 삭제한다. 처리 완료된 신고 기록은 조치 시점부터 3개월 보관한 뒤 삭제하며, 회원 탈퇴 시 계정·추천 참조를 제거해 익명화한다. 미처리 신고는 보관하되 관련 계정 탈퇴 시 함께 삭제한다. |
| 회원 탈퇴 | 계정 정보, 추천, Vote와 로그인 session을 삭제한다. 추천 삭제로 참조되지 않는 Track도 함께 정리한다. |
...
| 음악 검색 | 첫 provider adapter는 Apple iTunes Search API다. `KR` 검색을 우선하고 결과가 없을 때 `US` 검색은 후보 ID 발견에만 사용한다. 사용자에게 표시하고 등록하는 메타데이터는 모두 `KR` lookup으로 재확인된 결과만 사용한다. Explicit 곡을 포함한 관련도순 상위 20곡을 표시하고 Explicit 여부를 명시한다. |
...
| 한줄평 이력 | 한줄평은 작성 후 사용자가 수정하지 못한다. 곡 상세는 최초 한줄평을 계속 보존하고 Recommendation 회차가 2개 이상이면 가장 최근 회차의 한줄평 하나를 추가로 표시한다. |
| 나의 활동 | 인증 사용자에게 본인의 Recommendation 회차, 최초 등록 여부, 각 회차가 받은 Vote 수, 누적 수치와 최고 Vote 곡을 최신순으로 제공한다. |
| 의견 접수 | 비회원도 오류, UI·사용성, 기타 중 하나를 선택해 자유 의견을 제출할 수 있다. 분류, 본문과 접수 시각만 저장하고 계정, 이메일 또는 IP와 연결하지 않는다. |
...
- 본인 전용 `나의 활동` 대시보드와 추천 로그
- 공개 `의견 보내기` 화면과 익명 의견 저장
...
MVP 이후 검토할 기능과 미확정 정책은 [`product/post-mvp-backlog.md`](product/post-mvp-backlog.md)에서 관리한다.
