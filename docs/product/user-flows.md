...
| `/activity` | 나의 활동 | 인증 | 추천 로그, 최초 등록, 받은 Vote와 최고 반응 곡 |
| `/feedback` | 의견 보내기 | 공개 | 오류, UI·사용성, 기타 분류와 자유 의견 제출 |
...
- Recommendation 회차가 2개 이상이면 가장 최근 회차의 공개 닉네임, 한줄평과 추천 시각 하나
...
- 곡명, 아티스트 또는 둘을 함께 입력
- 명시적 검색 버튼과 Enter 지원
- `KR` storefront를 기본으로 사용한다. 빈 결과일 때 `US` 검색으로 후보 ID를 찾을 수 있지만 화면에는 `KR` lookup으로 확인된 메타데이터만 관련도순 상위 20곡까지 표시한다.
...
닉네임 변경과 Taste Profile은 표시하지 않는다. 준비 중인 기능을 빈 메뉴로 노출하지 않는다.

### S-09. My Activity

목적: 사용자가 자신이 소개한 음악과 그 추천 회차가 받은 반응을 개인 기록으로 확인한다.

콘텐츠:

- Recommendation 회차 수, 최초 등록 수와 받은 Vote 합계
- Vote가 가장 많았던 Recommendation 회차의 곡과 Vote 수
- 최신순 추천 로그 20개와 더 보기
- 각 회차의 곡, 아티스트, 추천 날짜, 한줄평, Vote 수와 최초 등록 여부

Vote 수는 해당 Recommendation의 `recommended_on`과 같은 날짜에 같은 Track이 받은 Vote만 집계한다. 같은 곡을 다른 날 다시 추천한 회차는 별도 항목으로 표시한다. 이 화면은 본인만 조회할 수 있으며 경쟁 순위나 공개 프로필 점수는 제공하지 않는다.

타인이 만든 Vote만 받은 Vote로 집계한다. 모든 회차의 받은 Vote가 0이면 최고 반응 곡 영역을 표시하지 않는다.

### S-10. Feedback

목적: 로그인 여부와 관계없이 서비스의 오류와 개선 의견을 간단히 전달한다.

콘텐츠:

- 제목 `TrackPick 의견 보내기`와 자유로운 의견을 요청하는 안내
- `오류`, `UI / 사용성`, `기타` 중 하나를 고르는 분류
- 최대 2,000자의 자유 입력란
- 이름과 이메일 등 개인정보를 적지 않도록 하는 안내
- 제출 성공 후 `의견을 남겨주셔서 감사합니다.` 완료 상태

의견에는 계정, 이메일 또는 IP를 연결하지 않는다. 분류와 본문, 접수 시각만 저장하며 로그인하지 않은 사용자도 제출할 수 있다.
...
| 의견 제출 | `ERROR`, `UI_USABILITY`, `OTHER` 분류와 1~2,000자 본문. 사용자 식별자는 보내지 않음 |
